...
**Token de conexão.** Criado com `clientUserId` igual ao `usuarioId` LançAI — a conexão bancária pertence ao usuário/conta principal, não ao Workspace. O Workspace é visão/organização; no schema atual `open_finance_conexao.workspace_id` ainda é pouso técnico (materialização de conta/movimento no workspace ativo) e será migrado em follow-up. Vai junto `avoidDuplicates: true`, que faz o provedor recusar conectar duas vezes a mesma credencial, e o `webhookUrl`, que amarra os eventos ao item recém-criado sem depender de configuração no painel do provedor.
