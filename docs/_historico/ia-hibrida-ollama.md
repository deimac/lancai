...
>
> - Papel dos provedores, circuit breaker e health-check: [10-IA.md](../10-IA.md)
> - Configuração local, produção e por que não há fila: [15-OPERACAO.md](../15-OPERACAO.md)
>
...
Antes de cada tentativa, o orquestrador faz um **health-check leve** (métricas/cache):
- Ollama: `GET /api/tags`
- Groq/Gemini/OpenAI/OpenRouter: listagem barata de modelos
- Em produção é **fail-open** (ping/falha recente não bloqueiam). Quem pausa provedor instável é o circuit breaker; se todos estiverem abertos, há uma limpeza e nova tentativa.
