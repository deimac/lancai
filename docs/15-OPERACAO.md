...
- `POST /cron/open-finance-importar-historico` — **importa extrato via GET** (já coletado no provedor). **Não** chama `PATCH /items` nem sync em lote (proibido pela Pluggy). Cobre Meu Pluggy (item que recusa update via API) e webhooks silenciosos. Antes do import, sonda **todas** as conexões listadas com `GET /items/{id}` (como no conectar). Item sumiu → `status: removida` e log `SYNC_ITEM_GONE` (não é `SYNC_FAIL`). Depois importa só conexões **stale** (`OPEN_FINANCE_STALE_AFTER_MINUTES`, padrão 240). Lookback GET: `OPEN_FINANCE_SYNC_LOOKBACK_DAYS` (padrão 14; primeira sync = 365). Lock em memória por conexão (não cruza com “Atualizar agora”). Logs `SYNC_START` / `SYNC_OK` / `SYNC_FAIL` / `SYNC_ITEM_GONE` / `SYNC_SKIP_FRESH` / `SYNC_SKIP_LOCKED`. `?dryRun=1` lista candidatas stale; `?limite=N` (máx. 200, padrão 50). **Agendar a cada 1–2 h** no Coolify (ou 6 h se volume baixo). Sem Fonte ativa responde `fonteAtiva: false` sem erro.
